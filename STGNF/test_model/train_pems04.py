...
d = dict()
d['start'] = pd.Timestamp('2018-01-01 00:00:00', freq='5T')
d['target']=train
...
d = dict()
d['start'] = pd.Timestamp('2018-02-10 00:00:00', freq='5T')
d['target']=val
...
d = dict()
d['start']= pd.Timestamp('2018-02-20 00:00:00',freq='5T')
d['target']=test
...
for i in train_data:
    print(i)
    print(len(i['target'][0]))
    print("train data:",i['target'].shape)
    print("train data:", i['target'].shape)
for i in val_data:
    #print(len(i['target'][0]))
    print("val data:",i['target'].shape)
for i in test_data:
    print(i)
    print("test data:",i['target'].shape)
...
import pickle
with open('../data_store/PEMS04/n_embedding.pkl', "rb") as file:
...
estimator = STGNFEstimator(
    model_parameters = {
        ...
        "mean":mean.reshape(307,3).to(device),
        "std":std.reshape(307,3).to(device),
